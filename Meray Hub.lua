...
local window = DrRayLibrary:Load("Meray Hub", "Default")

-- Create the first tab with an image ID
local tab1 = DrRayLibrary.newTab("information", "ImageIdHere")

-- Add elements to the first tab
tab1.newLabel("Information")
tab1.newButton("My discord", "join the creator's discord - coppy to browser", function()
    local link = "https://discord.gg/CCm8Y9VW"
    
    if setclipboard then
        setclipboard(link)
        print("The link has been copied to your clipboard!")
    else
        print("Your platform does not support 'setclipboard'.")
    end
end)

-- Create the second tab with a different image ID
local tab2 = DrRayLibrary.newTab("Main", "ImageIdLogoHere")

-- Add elements to the second tab
tab2.newDropdownWeapons("Weapons Farm", "Select Weapons!", {"Melee", "Sword", "Gun", "Blox Fruit"}, function(selectedOption)
    newDropdownWeapons:SetValue('Melee')
    newDropdownWeapons:OnChanged(function(Value)
        ChooseWeapon = Value
    end)
    task.spawn(function()
        while wait() do
...
tab2.newLabel("Farm")
tab2.newToggle("AutoFarmLevel", "Farm to max level!", function()
