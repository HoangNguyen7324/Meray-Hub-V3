...
local Window = redzlib:MakeWindow({
  Title = "Meray HUB : Blox Fruits",
  SubTitle = "by Merayliewz",
  SaveFolder = "Meray Hub | Meray_Hub.lua"
})
...
Discord:AddDiscordInvite({
  Name = "Meray Hub | Community",
  Description = "Join our discord community",
  Logo = "rbxassetid://125166504393992",
  Invite = "https://discord.gg/CCm8Y9VW"
})
local MainFarm = Window:MakeTab({"Farm", "Home"})
if Sea3 then
  local AutoSea = Window:MakeTab({"Sea", "Waves"})
  AutoSea:AddSection({"Kitsune"})
  local KILabel = AutoSea:AddParagraph({"Kitsune Island : Không xuất hiện"})
  AutoSea:AddToggle({Name = "Tự động spawn đảo Kitsune",Callback = function(Value)
    getgenv().AutoKitsuneIsland = Value;AutoKitsuneIsland()
  end})
  AutoSea:AddToggle({Name = "Tự động trao đổi Azure Ember",Callback = function(Value)
  ...
  task.spawn(function()
    ...
    while task.wait() do
      if Map:FindFirstChild("KitsuneIsland") then
        KILabel:SetTitle("Kitsune Island : Xuất hiện | Distance : " .. Distance)
      else
        KILabel:SetTitle("Kitsune Island : Không xuất hiện")
      end
    end
  end)
  AutoSea:AddSection({"Sea"})
  AutoSea:AddToggle({Name = "Tự động cày sự kiện biển",Callback = function(Value)
    getgenv().AutoFarmSea = Value;AutoFarmSea()
  end})
  AutoSea:AddButton({Name = "Mua thuyền mới",Callback = function()
    BuyNewBoat()
  end})
